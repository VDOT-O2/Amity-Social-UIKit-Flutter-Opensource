...
class AmityChatPage extends NewBasePage {
          ...
          child: Stack(
            children: [
              BlocProvider(
                ...
                create: (context) => ChatPageBloc(channelId, userId, userDisplayName, avatarUrl, context.read<AmityToastBloc>(), context,
                    ...
                    builder: (context, state) {
                      if (state is ChatPageStateInitial && !isJustCreated) {
                        context.read<AmityToastBloc>().add(AmityToastLoading(
                            message: context.l10n.chat_loading, icon: AmityToastIcon.loading, bottomPadding: toastBottomPadding));
                      }
                      ...
                      final isLoadingUserAvatar = state.avatarUrl == null && state.userDisplayName == null;

                      final List<GlobalKey> itemKeys = List.generate(state.messages.length, (index) => GlobalKey());

                      final isScrollable = state.scrollController.hasClients && state.scrollController.position.maxScrollExtent > 0;
                              ...
                              if (isLoadingUserAvatar)
                                ...
                                GestureDetector(
                                  ...
                                  onTap: () {
                                    ...
                                    AmityUIKit4Manager.behavior.chatPageBehavior.onAvatarTap(context, avatarUrl, userId);
                                  },
                                  child: AmityUserAvatar.withChannelMember(channelMember: state.channelMember),
                                ),
                          ...
                          children: [
                            Container(
                              ...
                              child: Column(
                                children: [
                                  Visibility(
                                    visible: state.isLoadingMore && state.messages.isNotEmpty && state.useReverseUI,
                                    child: Container(
                                    ...
                                    child: Stack(
                                      children: [
                                        LayoutBuilder(builder: (context, constraints) {
                                          final firstItem = state.messages.isNotEmpty ? state.messages.first.message : null;
                                          if (firstItem != null) {
                                            context.read<ChatPageBloc>().add(ChatPageEventMarkReadMessage(message: firstItem));
                                          }
                                            ...
                                            itemBuilder: (context, index) {
                                              ...
                                              if (message != null) {
                                                if (message.data is MessageVideoData &&
                                                    ...
                                                    !state.localThumbnails.containsKey(message.uniqueId)) {
                                                  try {
                                                    final filePath = (message.data as MessageVideoData).getVideo().getFilePath;
                                                    if (filePath != null) {
                                                      context.read<ChatPageBloc>().add(ChatPageEventFetchLocalVideoThumbnail(
                                                          uniqueId: message.uniqueId!, videoPath: filePath));
                                                    }
                                                  } catch (e) {}
                                                } else if (message.data is MessageVideoData &&
                                                    message.syncState == AmityMessageSyncState.SYNCED &&
                                                    !state.localThumbnails.containsKey(message.uniqueId) &&
                                                    (message.data as MessageVideoData).thumbnailImageFile == null) {
                                                  try {
                                                    final filePath = (message.data as MessageVideoData).getVideo().fileProperties.fileUrl;
                                                    if (filePath != null) {
                                                      context.read<ChatPageBloc>().add(ChatPageEventFetchLocalVideoThumbnail(
                                                          uniqueId: message.uniqueId!, videoPath: filePath));
                                                    }
                                                ...
                                                if (bounceAnimator == null) {
                                                  return AmityMessageBubble(
                                                    key: message.uniqueId != null ? Key(message.uniqueId!) : itemKeys[index],
                                                    pageId: pageId,
                                                    message: message,
                                                    ...
                                                    onResend: (message) {
                                                      context.read<ChatPageBloc>().add(ChatPageEventResendMessage(message: message));
                                                    },
                                                    onReplyMessage: (replyingMessage) {
                                                      context.read<ChatPageBloc>().add(ChatPageReplyEvent(message: replyingMessage));
                                                    },
                                                            ...
                                                            return VisibilityDetector(
                                                              ...
                                                              child: AmityMessageBubble(
                                                                  ...
                                                                  Navigator.push(
                                                                    ...
                                                                    MaterialPageRoute(
                                                                      builder: (context) => FullTextScreen(
                                                                        fullText: text,
                                                                        displayName:
                                                                            (!isReplied) ? state.userDisplayName ?? "" : "Replied message",
                                                                        theme: theme,
                                                                      ),
                                                                    ),
                                                                  );
                                                                ...
                                                                onResend: (message) {
                                                                  context
                                                                      .read<ChatPageBloc>()
                                                                      .add(ChatPageEventResendMessage(message: message));
                                                                },
                                                                onReplyMessage: (replyingMessage) {
                                                                  context
                                                                      .read<ChatPageBloc>()
                                                                      .add(ChatPageReplyEvent(message: replyingMessage));
                                                                },
                                                                onEditMessage: (message) {
                                                                  context.read<ChatPageBloc>().add(ChatPageEditEvent(message: message));
                                                                },
                                                  ...
                                                  child: Align(
                                                    ...
                                                    child: Container(
                                                      ...
                                                      child: Text(
                                                        item.date ?? "",
                                                        style: TextStyle(
                                                            color: theme.baseColorShade1, fontWeight: FontWeight.w400, fontSize: 13),
                                                      ),
                                                    ),
                                                  ),
                                                );
                                        ...
                                        if (newMessage != null) _buildNewMessageNotification(context, state, newMessage),
                                        if (state.showScrollButton && isScrollable && newMessage == null)
                                  ...
                                  if (state.isUserBlocked)
                                    ...
                                    AmityMessageComposer(
                                      ...
                                      action: MessageComposerAction(
                                        onDissmiss: () {
                                          context.read<ChatPageBloc>().add(const ChatPageRemoveReplyEvent());
                                        },
                                        onMessageCreated: () {
                                          final chatBloc = context.read<ChatPageBloc>();
                                          final hadNoMessages = state.messages.where((item) => item.type == ChatItemType.message).isEmpty;
  ...
  void _handleMessageVisibility(BuildContext context, AmityMessage message, int index, VisibilityInfo info, ChatPageState state) {
